...
> 线上 Demo：<https://ems-demo-3k3i.onrender.com>

这是 Render 免费服务，实例空闲后会自动休眠。首次访问可能需要等待几十秒唤醒，页面打开后即可直接登录体验。

默认登录账号：

| 用户名 | 密码 | 角色 | 推荐先看 |
|---|---|---|---|
| `superadmin` | `123456` | 超级管理员 | 全量菜单、数据看板、员工管理、角色权限、系统设置 |

Demo 已内置本地完整演示数据，包含员工档案、组织字典、考勤、请假、薪资、流程台账、系统用户和操作日志。

推荐体验路径：

```text
登录 superadmin -> 数据看板 -> 员工管理 -> 流程管理 -> 薪资社保 -> 角色管理
```
...
仓库已提供 Render 免费部署配置，适合生成一个可直接访问体验的公开链接。当前线上 Demo 地址：

```text
https://ems-demo-3k3i.onrender.com
```

免费服务空闲后会休眠，首次访问可能需要等待几十秒唤醒。部署相关文件：
...
详细步骤见 [免费线上 Demo 部署](docs/free-deploy.md)。

## 测试账号

默认登录账号建议使用 `superadmin / 123456`。所有演示账号密码均为 `123456`。
